...
const Dashboard = () => {
  ...
  const fetchStats = async (projectId = null) => {
    try {
      const url = projectId ? `/api/tasks/stats?projectId=${projectId}` : '/api/tasks/stats';
      const response = await axios.get(url);
  ...
  const handleProjectSubmit = async () => {
    ...
    try {
      if (projectModal.data) {
        const response = await axios.put(`/api/projects/${projectModal.data.id}`, projectFormData);
        setProjects(projects.map(project => project.id === projectModal.data.id ? response.data : project));
        toast.success("Project updated successfully");
      } else {
        const response = await axios.post('/api/projects', projectFormData);
    ...
    <PrivateRoute>
      <div className="min-h-screen bg-gradient-to-br from-gray-50 via-gray-50 to-blue-50/30 dark:from-gray-900 dark:via-gray-900 dark:to-blue-950/20">
        ...
        <DashboardNavbar
          ...
          onToggleSidebar={() => setSidebarState(prev => prev === 'hidden' ? 'full' : 'hidden')}
        />
        ...
        <main className={`flex-1 overflow-x-hidden transition-all duration-300 ${sidebarState === 'full' ? 'lg:pl-64' :
